...
function useCarouselScrollState(api: CarouselApi | undefined) {
  ...
  const getScrollPrev = React.useCallback(() => {
    return api?.canScrollPrev() ?? false
  }, [api])

  const getScrollNext = React.useCallback(() => {
    return api?.canScrollNext() ?? false
  }, [api])

  const canScrollPrev = React.useSyncExternalStore(subscribe, getScrollPrev, () => false)
  const canScrollNext = React.useSyncExternalStore(subscribe, getScrollNext, () => false)
